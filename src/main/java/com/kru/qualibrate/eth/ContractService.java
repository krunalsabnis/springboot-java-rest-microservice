...
public class ContractService {
	...
	public TransactionReceipt transact(String apiEP) {
		...
		try {
			log.info("web3j.ethGasPrice().send().getGasPrice() {}" , web3j.ethGasPrice().send().getGasPrice());
	...
	public String deployContract(Contract cont) throws IOException {
		String contractAddress = null;
		RawTransaction rawTransaction = RawTransaction.createContractTransaction(
				cont.getNonce(), cont.getGasPrice(), cont.getGasLimit(), cont.getValue(), 
				cont.getAbiByteCode());
		String transactionHash  = web3j.web3Sha3(rawTransaction.getData()).send().getResult();
